...
MD5::MD5(const void* data, size_t length) {
  reset();
  update(data, length);
}
...
std::string MD5::to_string() {
  const uint8_t* ptr = digest();
  return bytes_to_hex_string(ptr, 16);
}
...
void MD5::update(const void* data, size_t length) {
  update((const uint8_t*)data, length);
}
...
void MD5::update(std::ifstream& in) {
  ...
  while (!in.eof()) {
    ...
    length = in.gcount();
    if (length > 0) {
      update((const uint8_t*)buffer, (size_t)length);
    }
  }
...
void MD5::update(const uint8_t* data, size_t length) {
  ...
  memcpy(&buffer_[index], &data[i], length - i);
}
...
std::string MD5::bytes_to_hex_string(const uint8_t* input, size_t length) {
  std::string str;
  str.reserve(length << 1);
  for (size_t i = 0; i < length; ++i) {
    str.append(1, HEX[input[i] / 16]);
    str.append(1, HEX[input[i] % 16]);
  }
  return str;
}
